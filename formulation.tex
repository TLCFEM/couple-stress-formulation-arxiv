...
\usepackage{setspace,tikz,float,booktabs,amsmath,amssymb,mathpazo,siunitx}
...
\end{gather}
In which, $\mathbold{R}$ is used to denote residual. As can be seen later, it is possible to compute resistance based on \eqsref{eq:stationary}.
\subsection{Solution Procedure}
\subsubsection{Option One --- Invertible $\mathbold{E}_1$ and $\mathbold{E}_2$}
Since $\mathbold{E}_1$ and $\mathbold{E}_2$ are guaranteed to be square matrices, typically they are invertible with sufficient integration points provided, performing static condensation leads to the following result.
\begin{gather}
\begin{bmatrix}
...
Let the size of $\mathbold{H}_5$ be $i\times{}j$, the size of $\mathbold{H}_4$ be $n\times{}m$ and the number of strain and mean curvature components be $d$, then the minimum number of integration points $n_{ip}$ must satisfy the following inequalities,
\begin{gather}
n_{ip}\cdot{}d\geqslant{}\max\left(i,~j,~n,~m\right).
\end{gather}
...
\begin{gather}
n_{ip}\cdot{}d\geqslant{}n_k-n_f.
\end{gather}
Eventually,
\begin{gather}\label{eq:stability}
n_{ip}\cdot{}d\geqslant{}\max\left(i,~j,~n,~m,~n_k-n_f\right).
\end{gather}
...
\end{bmatrix}.
\end{gather}
The first term solely depends on conventional displacement, strain and stress as defined in Cauchy theory. Thus it can be derived from the Hu--Washizu theorem.
...
\paragraph{Stability Condition}
This option requires the inversion of $\mathbold{H}_4$ and $\mathbold{H}_5$ thus the stability condition is identical to \eqsref{eq:stability}.
...
Although the second option appears to be more efficient and possesses a form similar to that of displacement based elements, both options can be chosen to formulate elemental stiffness matrix and they share the same stability condition. However, they may require different numbers of shape functions for the corresponding fields in order to construct square matrices. Depending on the type of element and the number of available integration points, one may switch from one option to another for optimal formulation.
\subsection{Some Basic Elements}
Two elements named as couple stress triangle and quadrilateral (CSMT and CSMQ) are formulated as examples. Since the incremental form and the elemental stiffness are given in \eqsref{eq:incremental_form} and \eqsref{eq:stiffness}, there is no difficulty in constructing other types of elements such as serendipity quadrilaterals with various nodes, axisymmetric triangle/quadrilateral, eight-node brick/cube, four-node tetrahedron and higher order elements. The procedure follows a standard FEM approach.
\subsubsection{An Elementary Membrane Element --- CSMT}
...
The explicit forms are not listed here for brevity. Interested readers are referred to typical textbooks on finite element methods for details of formulating interpolation matrices \citep[see, e.g.,][section 5.1.3.1]{Zienkiewicz2013}. Three integration points are used for numerical integration. This element is denoted as couple stress mixed triangle (CSMT) element.

The CSMT element is essentially an extension of constant strain triangle (CST) element (CPS3 and CPE3 in ABAQUS notion) with additional fields incorporates mean curvature and couple stress. Given that only linear and constant interpolations are used, its performance shall be similar to that of CST.
\subsubsection{A Four-Node Quadrilateral Membrane Element --- CSMQ}
